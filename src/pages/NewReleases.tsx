...
import { useMusic } from '@/context/MusicContext'; // Import useMusic hook

const NewReleases = () => {
  const navigate = useNavigate();
  const { playSong } = useMusic(); // Use playSong from useMusic hook
  ...
  useEffect(() => {
    const fetchNewReleases = async () => {
      ...
      try {
        ...
        const data = await musicApi.getNewReleases('tamil'); // Correctly using getNewReleases
        setNewReleases(data || []);
      } catch (error) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {newReleases.map((song) => {
              ...
              return (
                <div 
                  key={song.id}
                  onClick={() => playSong(song, newReleases)} // playSong is now accessible
