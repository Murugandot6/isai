...
export const musicApi = {
  ...
  // Get trending songs for a given language (used for the main spotlight)
  getTrending: async (language: string = 'tamil'): Promise<Song[]> => {
    try {
      const response = await fetch(`${BASE_URL}/api/songs/trending?language=${encodeURIComponent(language)}&limit=20`);
      const res = await response.json();
      const data = res.data;
      if (!data) return [];
      const results = data.results || (Array.isArray(data) ? data : []);
