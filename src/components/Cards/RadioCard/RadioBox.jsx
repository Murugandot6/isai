...
const RadioBox = ({ radio, show, handleClick }) => {
    ...
    const { data: radioData, isFetching, error } = useSearchSongsQuery(radio.name);
    const [radioTracks, setRadioTracks] = useState([])

    useEffect(() => {
        setRadioTracks(getData({ type: 'tracks', data: radioData?.data?.results, library }));
    }, [library, radioData]);
    ...
    return (
        <>
            ...
            <div className="flex relative items-end justify-start gap-4 flex-wrap">
                <img src={radio.image} alt={radio.name} className="z-[0] absolute top-[-100px] left-[-100px] w-[50%] max-w-[360px] aspect-square blur-[70px] opacity-20" />
                <img className={`shadow-lg shadow-black/50 rounded-[10px] ${show && 'w-[150px] aspect-square'}`} src={radio.image} alt={radio.name} />
