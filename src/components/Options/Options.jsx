...
const calculateModalPosition = (buttonRect, modalWidth, modalHeight) => {
    ...
    let left = buttonRect.x - modalWidth - 400; // Further increased offset for 'more left'

    // Attempt to place higher than vertically centered, with a much larger offset
    let top = buttonRect.y + (buttonRect.height / 2) - (modalHeight / 2) - 400; // Further increased offset for 'more up'
        ...
        <div className="relative">
            ...
            {showModal && (
                <>
                    ...
                    <div className="fixed inset-0 z-[9999999998] bg-transparent" onClick={closeModal} />
                    <ul
                        ref={modalRef}
                        ...
                        className="shadow-xl overflow-hidden shadow-black/20 z-[9999999999] w-max flex-col text-gray-200 text-sm font-semibold rounded-[20px] bg-[#202020] min-w-[160px]"
                    >
                        {
                            filteredOptions
                                .map((option, index) =>
