...
const SecondLinks = ({ active, link, isMobileSidebar }) => {
    return (
        <li className={`${active ? `lg:bg-transparent transition-transform lg:text-white rounded-md` : 'translate-y-[8px] lg:translate-y-0'} ${link.color}`}>
            <NavLink 
                to={link.to} 
                className={`relative w-[70px] h-[50px] lg:w-auto p-2 lg:p-[6px] lg:h-[40px] rounded-[15px] text-xs lg:text-[1em] flex ${isMobileSidebar ? 'flex-row justify-start gap-4' : 'flex-col lg:flex-row justify-center lg:justify-start lg:gap-6'} items-center lg:text-gray-400 hover:lg:text-white font-semibold lg:font-normal lg:hover:bg-white/5 ${active && 'lg:bg-white/5'}`}
            >
                ...
                <span className={`relative transition-transform ${isMobileSidebar ? 'block' : (active ? 'lg:text-gray-100 block' : 'scale-0 lg:scale-100')} lg:flex-1 text-xs lg:text-sm lg:block`}>{link.name}</span>
            </NavLink>
        </li>
    )
};
